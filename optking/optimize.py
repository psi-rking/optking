...
def optimize(oMolsys, computer):
    ...
    computer: compute_wrappers.ComputeWrapper
        either Psi4Computer or QCEngineComputer
    Returns
    -------
    tuple or dict
    ...
    logger.debug(f"Running optimize")
    ...
    try:
        # Try to optimize one structure OR set of IRC points.
        # OptError and all Exceptions caught below.
        ...
        if op.Params.opt_type == 'IRC':
            ...
            IRCdata.history.set_step_size_and_direction(op.Params.irc_step_size,
                                                        op.Params.irc_direction)
        ...
        while not converged:
            try:
                ...
                # Could be erased after AlgError
                if not oMolsys.intcos_present:
                    make_internal_coords(oMolsys)
                    logger.debug("Molecular systems after make_internal_coords:")
                    logger.debug(str(oMolsys))
                ...
                if op.Params.opt_type == 'IRC':
                    if irc_step_number == 0:
                        ...
                        H, gX = get_pes_info(H, computer, oMolsys, stepNumber, irc_step_number)
                        ...
                        # atom_b = intcosMisc.wilson_b_mat(oMolsys.intcos, x_0)
                        # f_q = intcosMisc.q_forces(q_0, x_0, gX, atom_b)
                        ...
                        G = oMolsys.wilson_g_mat(oMolsys.masses)
                ...
                for stepNumber in range(op.Params.alg_geom_maxiter):
                    ...
                    H, gX = get_pes_info(H, computer, oMolsys, stepNumber, irc_step_number)
                    E = computer.energies[-1]
                    # haven't changed oMolsys shouldn't need to reassign xyz
                    ...
                    # atom_b = intcosMisc.wilson_b_mat(oMolsys)
                    # logger.debug(print_mat_string(atom_b, title="atom_b matrix"))
                    ...
                    if lastStepOK:
                        ...
                        elif (history.History.consecutiveBacksteps <
                              op.Params.consecutiveBackstepsAllowed):
                    ...
                    intcosMisc.apply_fixed_forces(oMolsys, f_q, H, stepNumber)
                    f_q, H = intcosMisc.project_redundancies_and_constraints(oMolsys, f_q, H)
                    oMolsys.q_show()

                    if op.Params.opt_type == 'IRC':
                        # DqGuess = IRCdata.history.q_pivot() - IRCdata.history.q()
                        # E and DqGuess were not used in method
                        Dq = IRCfollowing.dq_irc(oMolsys, f_q, H, op.Params.irc_step_size)
                    else:  # Displaces and adds step to history.
                        Dq = stepAlgorithms.take_step(oMolsys, E, f_q, H, op.Params.step_type,
                                                      computer)

                    if op.Params.opt_type == "IRC":
                        converged = convCheck.convCheck(stepNumber, oMolsys, Dq, f_q,
                                                        computer.energies,
                        ...
                        if converged:
                            ...
                            IRCdata.history.add_irc_point(irc_step_number, q_irc_point,
                                                          oMolsys.geom, forces_irc_point,
                                                          np.multiply(-1, gX),
                                                          computer.energies[-1],
                ...
                if op.Params.opt_type == 'IRC':
                    ...
                    if irc_step_number == op.Params.irc_points:
                        logger.info(
                            f"\tThe requested {op.Params.irc_points} IRC points have been obtained.")
                        raise IRCendReached()
                    else:
            ...
            # changing run-levels, optimization parameters, etc. and start over again.
                ...
                if AF.linearBends:
                    ...
                    eraseIntcos = True
                    eraseHistory = True
                    op.Params.update_dynamic_level_parameters(op.Params.dynamic_level)
        ...
        if op.Params.opt_type == 'linesearch':
            ...
            logger.info("\tGeometry and Gradient at end of linesearch")
            logger.info(print_geom_grad(oMolsys.geom, gX))
            del gX
...
def make_internal_coords(oMolsys):
    ...
    if op.Params.frag_mode == 'SINGLE':
        ...
        oMolsys.consolidate_frags()  # collapse into one frag (if > 1)
        oMolsys.split_frags_by_connectivity()  # separate by connectivity
        # increase connectivity until all atoms are connected
        oMolsys.augment_connectivity_to_single_frag(connectivity)
        oMolsys.consolidate_frags()  # collapse into one frag
        ...
        if oMolsys.num_frags == 1:
            oMolsys.split_frags_by_connectivity()

        if oMolsys.num_frags > 1:
            addIntcos.add_dimer_frag_intcos(oMolsys)
            # remove connectivity so that we don't add redundant coordinates
            # between fragments
            oMolsys.purge_interfrag_connectivity(connectivity)
