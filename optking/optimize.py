            ...
            for stepNumber in range(op.Params.geom_maxiter):
                ...
                if stepNumber == 0:
                    if op.Params.full_hess_every > -1:
                        ...
                        H = intcosMisc.convertHessianToInternals(Hcart, Molsys.intcos, xyz,
                                                                 Molsys.masses)
                    else:
                    ...
                    if op.Params.full_hess_every < 1:    # that is, compute hessian never or only once.
                        ...
                        xyz = Molsys.geom.copy()
                        Hcart = fHessian(
                            xyz, printResults=False)    # it's possible function moves geometry
                ...
                if op.Params.opt_type == 'IRC':
                    ...
                    Dq = IRCFollowing.Dq(Molsys, g, E, Hq, B, op.Params.irc_step_size, qPrime,
                                         dqPrime)
