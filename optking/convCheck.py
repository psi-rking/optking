...
def conv_check(iterNum, oMolsys, dq, f, energies, q_pivot=None):
    ...
    if op.Params.opt_type == "IRC":
        ...
        # Ginv_p = np.array(o_molsys.Nintco, float)
    ...
    if op.Params.opt_type != "IRC2":
        ...
        if iterNum == 0:
            ...
            conv_str += "\n"
        conv_str += "\t  %4d %16.8f  %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  %10.2e %1s  ~\n" % (
            iterNum + 1,
            energy,
            DE,
            ("*" if fabs(DE) < op.Params.conv_max_DE else "") if op.Params.i_max_DE else "o",
            max_force,
            ("*" if fabs(max_force) < op.Params.conv_max_force else "") if op.Params.i_max_force else "o",
            rms_force,
            ("*" if fabs(rms_force) < op.Params.conv_rms_force else "") if op.Params.i_rms_force else "o",
            max_disp,
            ("*" if fabs(max_disp) < op.Params.conv_max_disp else "") if op.Params.i_max_disp else "o",
            rms_disp,
            ("*" if fabs(rms_disp) < op.Params.conv_rms_disp else "") if op.Params.i_rms_disp else "o",
        )
        conv_str += (
            "\t----------------------------------------------------------------------" + "-----------------------\n\n"
        )
...
def test_for_convergence(DE, max_force, rms_force, max_disp, rms_disp):
    ...
    converge_gau = (
        converge == "GAU" or converge == "GAU_TIGHT" or converge == "GAU_VERYTIGHT" or converge == "GAU_LOOSE"
    )

    min_requirements = conv_max_force and (conv_max_DE or conv_max_disp)
    min_requirements_gau = flat_potential or (conv_max_force and conv_rms_force and conv_max_disp and conv_max_disp)
    ...
    # if criterion not active or criterion met, convergence!
        ...
        (conv_max_DE or DE_inactive)
        and (conv_max_force or max_f_inactive)
        and (conv_rms_force or rms_f_inactive)
        and (conv_max_disp or max_disp_inactive)
        and (conv_rms_disp or rms_disp_inactive)
    ):
        return True
    elif (
        op.Params.opt_type == "IRC"
        and (DE_inactive or conv_max_DE)
        and (max_disp_inactive or conv_max_disp)
        and (rms_disp_inactive or conv_rms_disp)
    ):
        return True
    else:
        return False
