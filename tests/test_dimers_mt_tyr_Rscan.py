...
def test_dimers_mt_tyr_Rscan():
    ...
    dimer = {
      ...
      'A Frag': 1,
      'A Ref Atoms': [[1,3,4,6,8], # Average of position of ring atoms
                      [8],         # S, shouldn't matter much which chosen
                      [11]],       # H, best to avoid a collinear A3-A2-A1 angle,
                                   # also, avoid phi_B ~180 deg
      'A Label': 'methylthiophene', # labels are optional
      'B Frag': 2,
      'B Ref Atoms': [[13,14,15,16,17,18], # Average of ring C's
                      [13],        # one C 
                      [15]],       # another C
    ...
    (R, theta_A, theta_B, tau, phi_A, phi_B) = [5.03575,
        94.64, 86.36, -46.50, 167.38, -97.60]
    ...
    for R_AB in np.arange(4.0, 8.1, 1.0):
        ...
        Bxyz[:] = dimerCoord.orient_fragment(Axyz, Bxyz, q_target, unit_angle='deg',
